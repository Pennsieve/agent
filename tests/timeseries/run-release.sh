...
WAIT_TIME=2
PSAGENT="target/release/pennsieve"
...
LOCAL_PORT=$(PSAGENT config show timeseries_local_port)
...
START_TIME=$(date '+%s')
node "$(realpath "$BASE/main.js")" "$LOCAL_PORT" "$TOKEN" "$(realpath "$BASE/request.json")"
TEST_EXIT_CODE=$?
END_TIME=$(date '+%s')
